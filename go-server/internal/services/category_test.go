...
func TestCircularReference(t *testing.T) {
	...
	// Create A
	a, err := svc.Create("A", "root", nil)
	if err != nil {
		t.Fatalf("create A failed: %v", err)
	}
	// Create B under A
	b, err := svc.Create("B", "child", &a.ID)
	if err != nil {
		t.Fatalf("create B failed: %v", err)
	}
	// Create C under B
	c, err := svc.Create("C", "grandchild", &b.ID)
	if err != nil {
		t.Fatalf("create C failed: %v", err)
	}
	// Attempt to move A under C (would create cycle)
	if err := svc.Update(a.ID, "A", "root updated", &c.ID); err == nil {
...
func TestDepthLimit(t *testing.T) {
	...
	if _, err := svc.Create("S", "sibling", &root.ID); err != nil {
