              ...
              <div class="p-4 flex-1 overflow-y-auto custom-scrollbar">
                ...
                <div class="flex gap-2 mb-4">
                  <input
                    ...
                    placeholder="新建分类..."
                    class="flex-1 bg-ms-deep text-slate-300 text-sm rounded-sharp px-3 py-2 border border-ms-border outline-none focus:border-brass placeholder-slate-600 transition-all"
                    @keyup.enter="handleCreateCategory"
                  />
                  <button
                    @click="handleCreateCategory"
                    :disabled="!newCategoryName.trim()"
                    class="flex items-center gap-1 text-xs bg-neon/10 text-neon px-3 py-2 rounded-sharp hover:bg-neon/20 transition-all border border-neon/30 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <Plus :size="14" />
                    <span>新建</span>
                  </button>
                ...
                <div
                  ...
                  class="mb-3 px-3 py-2 bg-amber-500/10 border border-amber-500/30 rounded-sharp text-xs text-amber-400"
                >
                  将在「{{
                  ...
                  <button
                    ...
                    class="ml-auto text-amber-300 hover:text-amber-100"
                  >
                    取消
                  </button>
                </div>
            ...
            <div class="w-3/5 flex flex-col">
              <div v-if="selectedId" class="p-6 flex-1 overflow-y-auto custom-scrollbar">
                ...
                <div class="mb-6">
                  ...
                  <div class="grid grid-cols-2 gap-3">
                    ...
                    <div class="bg-ms-deep border border-ms-border p-3">
                      <div class="text-2xl font-mono text-brass">{{ categoryChildCount }}</div>
                    ...
                    <div v-for="card in categoryCards" :key="card.id"
                      ...
                      <div class="relative shrink-0">
                        ...
                        <div v-if="movingCardId === card.id" @click.stop
                          class="absolute right-0 top-5 w-40 bg-ms-carbon border border-ms-border rounded-sharp shadow-xl z-20 max-h-40 overflow-y-auto">
                  ...
                  <div class="space-y-4">
                    <div>
                      ...
                      <input v-model="editName"
                        class="w-full bg-ms-deep text-slate-200 text-sm rounded-sharp px-4 py-2.5 border border-ms-border outline-none focus:border-brass transition-all"
                        placeholder="输入分类名称" />
                    </div>
                    <div>
                      <label class="block text-xs text-slate-400 mb-1.5">分类描述</label>
                      <textarea v-model="editDescription"
                        class="w-full bg-ms-deep text-slate-200 text-sm rounded-sharp px-4 py-2.5 border border-ms-border outline-none focus:border-brass transition-all resize-none h-20"
                    ...
                    <button @click="handleSave" :disabled="!editName.trim()"
                      class="w-full bg-transparent text-brass text-sm font-medium py-2 rounded-sharp border border-brass/50 hover:bg-brass/10 transition-all disabled:opacity-30 disabled:cursor-not-allowed">
                      保存更改
                    </button>
                  </div>
              ...
              <div v-else class="flex-1 flex items-center justify-center text-slate-600 text-sm">
                <div class="text-center">
                  <Settings :size="32" class="mx-auto mb-3 text-brass opacity-30" />
                  <p>选择左侧分类查看仪表盘</p>
                </div>
              </div>
            </div>
