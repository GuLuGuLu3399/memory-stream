    ...
    <div data-tauri-drag-region
        ...
        <div data-tauri-drag-region class="flex items-center pl-[68px] gap-0.5">
            ...
            <button @click="layoutStore.openMergeConsole()" title="概念坍缩引擎"
                ...
                :class="isMergeConsoleOpen ? 'text-amber-400' : 'text-slate-600 hover:text-amber-400'">
                <Merge :size="14" />
            </button>
            <button @click="layoutStore.openCategoryPanel()" title="分类档案库"
                class="w-7 h-7 flex items-center justify-center transition-colors"
                :class="isCategoryPanelOpen ? 'text-neon' : 'text-slate-600 hover:text-neon'">
                <FolderCog :size="14" />
            </button>
        ...
        <div class="flex items-center h-full">
            <button @click="minimize"
                class="h-full w-11 flex items-center justify-center text-slate-600 hover:bg-white/5 hover:text-slate-400 transition-colors"
                title="最小化">
