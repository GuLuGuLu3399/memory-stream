...
describe("useCategoryStore", () => {
  ...
  describe("createCategory", () => {
    ...
    it("should create category with parentId and return created id", async () => {
      mockInvoke.mockResolvedValueOnce({ category: { id: 99 } });
      mockInvoke.mockResolvedValueOnce(mockCategoriesResponse);

      const store = useCategoryStore();

      const createdId = await store.createCategory(
        "Child Category",
        "Child desc",
        1,
      );
