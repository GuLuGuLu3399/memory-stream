...
vi.mock("wasm-engine", () => ({
  ...
  process_markdown: vi.fn((rawMd: string) => ({ html: `<p>${rawMd}</p>`, ast_json: "{}" })),
}))
...
describe("useCards", () => {
  ...
  describe("loadDetail", () => {
    it("should return cached detail if available", async () => {
      const cached = { id: "card-1", title: "Cached", html: "<p>Cached</p>", rawMd: "Cached", updatedAt: "2024-01-01", tocData: null }
