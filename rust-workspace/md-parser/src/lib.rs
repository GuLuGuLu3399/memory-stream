...
/// 获取 wikilink 正则实例（延迟初始化，仅编译一次）。
fn wikilink_regex() -> MSResult<&'static Regex> {
...
/// 从 Markdown 文本中提取 wikilink（`[[link]]`）。
///
...
pub fn extract_wikilinks(md: &str) -> MSResult<Vec<String>> {
...
/// 将文本裂解为 Text/Wikilink 节点并按顺序挂载到当前父节点。
fn split_text_into_nodes<'a>(text: CowStr<'a>, top: &mut AstNode<'a>) -> MSResult<()> {
    ...
    match text {
        CowStr::Borrowed(s) => {
            ...
            for caps in re.captures_iter(s) {
                ...
                if full.start() > last_end {
                    top.push_child(AstNode::Text {
                        value: Cow::Borrowed(&s[last_end..full.start()]),
                    })?;
                }
                ...
                if !target.is_empty() {
                    let alias = caps
                        ...
                        .map(|m| m.as_str().trim())
                        .filter(|a| !a.is_empty());
                    top.push_child(AstNode::Wikilink {
                        target: Cow::Borrowed(target),
                        alias: alias.map(Cow::Borrowed),
                    })?;
                } else {
                    top.push_child(AstNode::Text {
                        value: Cow::Borrowed(full.as_str()),
                    })?;
                }
        ...
        _ => {
            ...
            let mut last_end = 0;
            let mut has_match = false;
            for caps in re.captures_iter(&owned) {
                has_match = true;
                let Some(full) = caps.get(0) else {
                    continue;
                };
                ...
                if full.start() > last_end {
                    top.push_child(AstNode::Text {
                        value: Cow::Owned(owned[last_end..full.start()].to_string()),
                    })?;
                }
            ...
            if has_match && last_end < owned.len() {
                top.push_child(AstNode::Text {
                    value: Cow::Owned(owned[last_end..].to_string()),
                })?;
            }

            if !has_match && !owned.is_empty() {
                top.push_child(AstNode::Text {
...
mod tests {
    ...
    #[test]
    fn test_parse_wikilink_as_structured_node() -> Result<(), Box<dyn std::error::Error>> {
        let md = "Hello [[World]]!";
        let ast = parse_markdown(md)?;
        let first_node = get_first_child(&ast);
    ...
    #[test]
    fn test_parse_wikilink_not_expanded_in_code_block() -> Result<(), Box<dyn std::error::Error>> {
        let md = "```js\nconst s = \"[[World]]\";\n```";
        let ast = parse_markdown(md)?;
        let first_node = get_first_child(&ast);
