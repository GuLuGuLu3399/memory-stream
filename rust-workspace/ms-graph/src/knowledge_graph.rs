...
use crate::error::{ErrorCode, GraphError, GraphResult};
use crate::model::{GraphEdge, GraphNode, SubgraphResult};
...
impl KnowledgeGraph {
    ...
    /// 添加有向边
    pub fn add_edge(&mut self, source_id: &str, target_id: &str, edge: GraphEdge) -> GraphResult<()> {
        let source = self.node_index.get(source_id).ok_or_else(|| {
            GraphError::new(ErrorCode::GraphNodeNotFound, format!("节点不存在: {source_id}"))
        })?;
        let target = self.node_index.get(target_id).ok_or_else(|| {
            GraphError::new(ErrorCode::GraphNodeNotFound, format!("节点不存在: {target_id}"))
        })?;
    ...
    /// BFS N 度子图提取
    pub fn subgraph(&self, center_id: &str, depth: usize) -> GraphResult<SubgraphResult> {
        let start = self.node_index.get(center_id).ok_or_else(|| {
            GraphError::new(ErrorCode::GraphNodeNotFound, format!("节点不存在: {center_id}"))
        })?;
    ...
    /// 批量构建图谱 — 从节点和边列表
    pub fn build_from(nodes: Vec<GraphNode>, edges: Vec<GraphEdge>) -> GraphResult<Self> {
        let mut graph = Self::new();
        for node in nodes {
            graph.add_node(node);
        }
        for edge in &edges {
            graph.add_edge(&edge.source, &edge.target, edge.clone())?;
        }
        Ok(graph)
    }
}
...
mod tests {
    ...
    fn make_node(id: &str, title: &str) -> GraphNode {
        GraphNode { id: id.to_string(), title: title.to_string(), x: None, y: None }
    }

    fn make_edge(source: &str, target: &str) -> GraphEdge {
        GraphEdge { source: source.to_string(), target: target.to_string(), relation: "link".to_string() }
    }

    #[test]
    fn test_add_nodes_and_edges() -> GraphResult<()> {
    ...
    #[test]
    fn test_subgraph_bfs() -> GraphResult<()> {
        let mut g = KnowledgeGraph::new();
        g.add_node(make_node("a", "A"));
        g.add_node(make_node("b", "B"));
    ...
    #[test]
    fn test_topological_layers() -> GraphResult<()> {
        let mut g = KnowledgeGraph::new();
        g.add_node(make_node("a", "A"));
        g.add_node(make_node("b", "B"));
        g.add_node(make_node("c", "C"));
        ...
        let layers = g.topological_layers()
            .ok_or_else(|| GraphError::new(ErrorCode::GraphCycleDetected, "topological sort failed"))?;
    ...
    fn test_build_from() {
        ...
        let g = KnowledgeGraph::build_from(nodes, edges).unwrap();
        assert_eq!(g.node_count(), 2);
        assert_eq!(g.edge_count(), 1);
    }
}
